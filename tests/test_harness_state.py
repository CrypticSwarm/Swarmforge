...
class WorktreeWrapperInstall(WorktreeCase):
    ...
    def stub_git(self, payload):
        """Put a stand-in git first on PATH and return its path.

        The wrapper records whatever `git` resolved to when it was written, so
        a stand-in staged before the write is the git the wrapper calls. It
        prints `payload` for a porcelain worktree listing and echoes its own
        argv otherwise, which is what tells the two branches apart.
        """
        stub_dir = os.path.join(self.tmp, "stubbin")
        path = os.path.join(stub_dir, "git")
        write_file(path, (
            '#!/bin/sh\n'
    ...
    def run_wrapper(self, *args):
        """The wrapper's own git, run with `args`."""
        return subprocess.run(
            [os.path.join(self.wrapper, "git")] + list(args),
            capture_output=True, text=True, check=True).stdout

    def test_the_wrapper_rewrites_the_recorded_host_path(self):
        """The rewrite is checked by running it, not by reading it.

        A wrapper asserted as source text passes whether or not its case glob
        matches the invocation git is actually given, and whether or not the
        sed anchor holds -- and the repair for either is to update the
        expected text, which records the break instead of failing on it.
        """
        listing = "worktree %s\nHEAD 0000\nbranch refs/heads/main\n" % (
            self.HOST_WORKTREE,)
        self.stub_git(listing)
        ...
        rewritten = self.run_wrapper("worktree", "list", "--porcelain")
        self.assertEqual(
            rewritten, "worktree %s\nHEAD 0000\nbranch refs/heads/main\n" % (
                self.ws,))
        self.assertNotIn(self.HOST_WORKTREE, rewritten)

    def test_the_wrapper_leaves_every_other_command_alone(self):
        """Only the porcelain listing is rewritten; the rest is exec'd."""
        self.stub_git("worktree %s\n" % self.HOST_WORKTREE)
        self.stage_linked_worktree(self.HOST_WORKTREE + "/.git")
        umask = os.umask(0o022)
        self.addCleanup(os.umask, umask)
