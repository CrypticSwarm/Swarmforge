#!/usr/bin/env python3
"""Tests for delivering generated tong MCP servers into the Grok config.

Run: python3 tests/test_merge_grok_mcp.py
...
from swarmforge.config import merge_grok_mcp
...
class MergeGrokMcpTests(unittest.TestCase):
    def _merge(self, config, fragment=None):
        stderr = io.StringIO()
        with contextlib.redirect_stderr(stderr):
            merge_grok_mcp.merge(config, fragment)
    ...
    def test_creates_config_with_managed_block(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            text = _read(config)
            self.assertIn(merge_grok_mcp.BLOCK_BEGIN, text)
            self.assertIn(merge_grok_mcp.BLOCK_END, text)
            self.assertEqual(
                tomllib.loads(text),
    ...
    def test_no_fragment_strips_block_and_keeps_user_config(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            text = _read(config)
            self.assertNotIn(merge_grok_mcp.BLOCK_BEGIN, text)
    ...
    def test_invalid_user_config_still_gets_block(self):
        # Grok will reject the broken config on its own; the merge only loses
        # the duplicate-name check and says so.
        with tempfile.TemporaryDirectory() as tmp:
            config = os.path.join(tmp, "config.toml")
            with open(config, "w", encoding="utf-8") as handle:
            ...
            self.assertIn(merge_grok_mcp.BLOCK_BEGIN, _read(config))
