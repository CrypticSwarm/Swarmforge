...
- If scope is `global`, write to `<global-config>/skills/<skill-name>/SKILL.md` where `<global-config>` is `opencode/config` if present in the workspace, otherwise `~/.config/opencode`.
...
- If scope is `global`, commands live in `<global-config>/command/` (use `opencode/config` if present in the workspace, otherwise `~/.config/opencode`).
