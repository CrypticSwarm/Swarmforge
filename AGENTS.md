...
- `anvil/` holds the container-side assets shared by every harness image: the Dockerfile, the container entrypoint, and the Claude-only status line (`statusline.sh`) with the image config layer that turns it on (`claude-settings.json`). The images build from the repo root (`-f anvil/Dockerfile`) so `swarmforge/` can be copied in; `.dockerignore` keeps everything else out of the context.
