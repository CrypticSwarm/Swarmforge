...
- `swarmforge/` is the Python for both sides of the container boundary: the host launcher (`anvil`, `tongs`, `gitguard`, `cli`, `worktrees`), and the agent translation and config merging the entrypoint runs with `python3 -m`. It is stdlib-only: the image installs no third-party Python and the launcher runs on the host's `python3`.
