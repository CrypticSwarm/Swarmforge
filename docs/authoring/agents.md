...
Subagent definitions live under `agents/` in this checkout, and in the agents dir of every other [asset layer](../configuration.md#asset-layers). One unified format is rewritten to each harness's native dialect at container startup, by `swarmforge/agents/translate.py` through the emitter the harness's spec declares.
