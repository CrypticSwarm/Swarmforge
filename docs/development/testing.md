...
The full variable list for each target is in [Environment variables](../reference/environment.md#tests-and-tooling).
...
Four files assert on the shape of the repo rather than on any one module. `tests/test_image_layout.py` holds the Dockerfile and the entrypoint to the same import root, `tests/test_package_layering.py` keeps the package's imports acyclic and keeps loading a module from a file path out of everything but the `bin/` shims, `tests/test_harness_conformance.py` holds every registered harness to the contract described under [Harness lifecycle](architecture.md), and `tests/test_docs_links.py` keeps every relative link in this tree and in the root `README.md` resolving, and every page reachable from [the index](../README.md). All four fail the way a build should — before anything reaches a container.
