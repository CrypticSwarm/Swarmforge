...
Folder trust keys on that same root, and Grok records it — and with it project instructions, skills, MCP and hooks — only when the root matches the directory it was started in.
In a linked worktree the root Grok resolves comes from the worktree's registration, which the [git guard](../git-guard.md) restates for the container.
